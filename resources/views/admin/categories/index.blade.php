              ...
              <table class="table table-striped table-bordered zero-configuration">
                ...
                <tbody>
                    @forelse ( $categories as $key=>$category )
                        <tr id="{{$category->id}}">
                            <td>{{$category->name}}</td>
                            <td>{{ $category->subCategory->name ?? "لايوجد تصنيف اساسي" }}</td>
                            <td>{{$category->created_at}}</td>
                            <td>
                                <form action="javascript:void(0)" method="post">
                                    @csrf
                                    <button data-id="{{ $category->id }}" data-name="{{$category->name}}" data-toggle="tooltip" data-placement="top" title="" data-original-title="حذف التصنيف"  class="btn  btn-danger del">
                                        <i class="la la-trash-o"></i>
                                    </button>
                                    <a href="{{route('admin.categories.edit',[$category->id])}}" class="btn btn-info" data-toggle="tooltip" data-placement="top" title="" data-original-title="تعديل التصنيف" ><i class="la la-edit"></i></a>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" align="center">لاتوجد تصنيفات  </td>
                        </tr>
                    @endforelse
                </tbody>
                <tfoot>
...
$(".del").on('click', function() {
    ...
    Swal.fire({
        ...
        icon:'info',
        title: "هل انتا متأكد؟",
        text: "سوف تقوم بحذف التصنيف : " +name,
        type: "warning",
